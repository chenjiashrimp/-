...
def accuracy(typed, reference):
    ...
    for i in range(min(len(typed_words),len(reference_words))):
        # print(typed_words[i],reference_words[i])
        if typed_words[i] == reference_words[i]:##== 和is
            corresponding+=1
    return corresponding*100/len(typed_words)
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        min_diff_word,min_limit=typed_word,100
        for word in valid_words:
            difference=diff_function(typed_word, word, limit)
            if difference <min_limit and difference<=limit:
                min_limit,min_diff_word=difference,word
...
def feline_flips(start, goal, limit):
    ...
    # END PROBLEM 6
# big_limit=10
# print(feline_flips("rose", "hello", big_limit))
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit==-1:  # Fill in the condition
        ...
        # END
# print(minimum_mewtations('eyed', 'ey', 1))


###
###要会用计算机思维去思考问题，比如说上面那个函数，人的思路是看，比较两个单词，然后凭感觉
###得出是要增、删还是改，但不能把这个解题思路用于编程，计算机没办法观察两个单词的“形状”
###也没有什么“感觉”，计算机能做的就是把三种修改方式一视同仁然后一种种尝试，最后得到最好的结果
###这个思路从人的角度来说很笨，但却符合计算机思维
###我一开始就想着如何编程，让计算机能跟人一样直接判断出用哪种修改方式，这明显是错误的
###你可没办法把“感觉”写成代码
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for i in range(len(sofar)):
        ...
        if i==len(sofar)-1:
            progress=len(sofar)
    upload({'id':user_id,'progress':progress/len(prompt)})
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if get_times(match)[j][i]<fastest_time:
                fastest_time=get_times(match)[j][i]
                fastest_play=j
        fastest_words[fastest_play].append(get_words(match)[i])
